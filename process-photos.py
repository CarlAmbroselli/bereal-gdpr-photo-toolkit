...
def update_exif(image_path, datetime_original, location=None, caption=None):
    try:
        ...
        exif_dict['Exif'][piexif.ExifIFD.DateTimeOriginal] = datetime_original.strftime("%Y:%m:%d %H:%M:%S").encode('utf-8')
        logging.info(f"Added capture date and time: {datetime_original.strftime('%Y:%m:%d %H:%M:%S')}")
        ...
        if caption:
            ...
            logging.info(f"Updated title with caption: {caption}")

        logging.info(f"Exif dict {exif_dict}.")
    ...
    try:
        ...
        for path, role in [(primary_path, 'primary'), (secondary_path, 'secondary')]:
            ...
            if convert_to_jpeg == 'yes':
                ...
                logging.info(f"EXIF data added to converted image. {new_path} | {taken_at} | {location} | {caption}")
